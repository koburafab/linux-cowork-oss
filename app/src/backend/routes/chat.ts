...
function resolveModelConfig(body: { model?: string }): ModelConfig {
	...
	if (config.provider === "anthropic") {
		...
		config.baseUrl?.includes("moonshot.ai") ||
...
export function createChatRoutes(toolRegistry: ToolRegistry): Hono {
	...
	app.post("/chat", async (c) => {
		...
		// CLI providers (subscription) run their OWN agentic loop — bypass the app's
		// tool loop and let the CLI act when agent mode (useTools) is on.
		const isCli =
